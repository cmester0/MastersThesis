...
\end{figure}
~\\ \\
% Inductive types can be defined as \texttt{W}-types, which are constructed as the initial algebra for a polynomial functor. We can dually define coinductive types as \texttt{M}-types, which are constructed as the final coalgebra for a polynomial functor.\todo{final coalgebra is scary words, find something more "noob" friendly} \todo{... algebra for functor} \todo{W-types, Induction , M-types, Coinduction...}
\todo[inline]{algebra for functor ? W-types? should there be a decription of these?}
...
We want to create a rule set for how to define coinductive types as \texttt{M}-types. We would like to be able to define a type from a given set of constructor / destructors. If we for example is given the rule
\begin{equation}
...
\end{equation}
which is the delay type. We can define some more involved constructors, that build on the type itself
\begin{equation}
...
We end this chapter by exploring a wacky \texttt{M}-type, that utilizes the power of containers to the fullest. We define a wacky \texttt{M}-type by the following container
\begin{equation}
...
\begin{defn}
  ...
  \end{center}
\end{defn}
\noindent Trees where the permutation of subtrees does not matter is multisets. We can define equality between permuation of subtrees by the following rule
\begin{equation}
...
\begin{thm}
  ...
  There is an equality between the types \(\typeformer{T}\,\type{A}/\relation{\sim_{T}}\) and \(\typeformer{MS}~\type{A}\), assuming the axoim of choice.
  \begin{proof}
...
We define sequences, and show they are equal to the delay monad (\thmref{seq-eq-delay}). We then extend this equality to an equality between the types quotient by weak bisimilarity (\corref{delay-to-seq-quotient}). We start by defining the type of sequences.
...
\end{thm}
\begin{cor}
  \label{cor:delay-to-seq-quotient}
  The types \(\typeformer{Delay}~\type{R}/\relation{\sim}\) and \(\typeformer{Seq}_{\type{R}}/\relation{\sim}\) are equal.
  \begin{proof}
...
In this section we will show that assuming the axiom of choice, we get an equivalence between sequences quotiented by weak bisimilarity and the partiality monad (\thmref{sequence-partiality-monad-eq}). We start by defining some relation on sequences.
\begin{defn}[Sequence Termination]
  ...
  \end{equation}
\end{defn}
\begin{defn}[Sequence Ordering] We can define an ordering of sequences as
  \begin{equation}
...
\begin{defn}[Maybe Ordering]
  ...
  We can define an ordering relation on \(\type{R} + \unit\) as 
  \begin{equation}
...
\begin{defn}
  ...
  We can define a function \(\function{Seq/{\sim}{\rightarrow}(-)_\bot}\) from \(\typeformer{Seq}_{\type{R}}\) to \(\type{R}_{\bot}\)
  \begin{equation}
...
\begin{lem}
  ...
  \begin{proof}
    ...
    \end{equation}
    and \(\function{P_{prop}}\) follows from the propositional truncation. For \(\constructor{\bot}\) and \(\constructor{\eta}~\term{a}\), we use \(\function{now}_{seq}~\term{a}\) and \(\constant{never}_{seq}\) respectively, and by \eqref{eq:constant-sequence-equality} we get \(\typeformer{P}~\constructor{\bot}\) and \(\typeformer{P}~(\constructor{\eta}~\term{a})\). For the least upper bound \(\constructor{\bigsqcup} (\function{s},\function{q})\), we may assume \(\prod_{(n : \mathbb{N})}~\typeformer{P}~(\function{s}~n)\) which gives us a pointwise equivalence
    \begin{equation}
  ...
  \end{equation}
  which by the axoim of choice becomes
  \begin{equation}
...
\begin{cor}
  ...
  The function \(\function{Seq/{\sim}{\rightarrow}(-)_{\bot}}\) is surjective assuming the axoim of choice.
  \begin{proof}
...
\begin{thm}
  ...
  \begin{proof}
 The function \(\function{Seq/{\sim}{\rightarrow}(-)_\bot}\) is injective (\lemref{sequence-to-partiality-monad-injectivity}) and surjective (\corref{sequence-partiality-monad-surjective}) assuming the axoim of choice, meaning we get an equivalence, since we are working in hSets.
\end{proof}
\end{thm}

\section{How should Q\texttt{M}-types be defined}
We want to define what a Q\texttt{M}-type means in general, we draw inspiration from QW-types \cite{DBLP:Constructing-QIITs}, quotient containers \cite{DBLP:quotient-container} and the quotient polynomial functors (QPF) \cite{DBLP:QM-lean}. From what we have seen as examples in the previous section, equality construction between Set Quotiented \texttt{M}-types and QIITs are given as
\begin{itemize}
...
\todo{"The category of containers lacks good coequalisers" - \cite{DBLP:CategoryOfContainers}}
An alternative to directly set quotienting the \texttt{M}-types we have defined, is to do the quotienting on the underlying container \cite{DBLP:quotient-container} / polynomial functor \cite{DBLP:QM-lean}, and then do the fixed point construction we did for polynomial functors, but on the quotiented functor instead. We start by defining quotients on containers.
\begin{defn}
  Given a container \(\containerpair{\type{A}}{\typeformer{B}}\), and a relation \todo{...} we can form a quotiented container \(\containerpair{\type{A}}{\typeformer{B}} / \relation{R}\) \todo{Complete definition}
\end{defn}
...
\todo{...} again we run into problems with the axiom of choice. The construction of QW types got around this problem, by doing the construction as a QIIT, we will try and follow the same ideas and show what that would look like for \texttt{M}-types. \todo{Complete construction of QW inspired Q\texttt{M}-types}

\subsection{Better behaving Q\texttt{M}-types}
If we take inspiration from the construction of Q\texttt{W}-types, and define the quotiented types as QIITs, by using a set of rules, we will get better behaving quotients, meaning we can construct the type, without assuming the axiom of choice. The rules for constructing the QIITs are
\begin{itemize}
\item 
\end{itemize}
\todo{QIITs}

...
\begin{exmp}
  ...
  \begin{figure}[h]
    ...
    \begin{tikzcd}[row sep=large,column sep=large]
      \type{A} & \type{A} \times \typeformer{M}_{\containerpair{\type{A}}{\lambda\,\_,\unit}} \ar[d,shift right,"\function{in}"'] \ar[l,"\pi_1"'] \ar[r,"\pi_2"] & \typeformer{M}_{\containerpair{\type{A}}{\lambda\,\_,\unit}} \\
      & \typeformer{M}_{\containerpair{\type{A}}{\lambda\,\_,\unit}} \ar[ul,"\destructor{hd}"] \ar[u,shift right,"\function{out}"'] \ar[ur,"\destructor{tl}"']
    \end{tikzcd}
...
\end{equation}
where each destructor \(\destructor{d_n} : \type{T} \rightarrow \type{F}_n\) for the field \(\term{f}_n\) will be defined as \(\destructor{d_n}~\term{t} = \pi_n~(\function{out}~\term{t})\). However fields in a coinductive record may depend on previous defined fields. That is we can have the list of fields \(\term{f}_1 : \type{F}_1,\term{f}_2 : \typeformer{F}_2,\cdots,\term{f}_n : \typeformer{F}_n\), where each field depends on all the previous once. We define this as the \texttt{M}-type for the container
\begin{equation}
...
\begin{equation}
  \containerpair{\sum_{(\type{f}_1 : \universe{\mathcal{U}})} \left(\unit \times \sum_{(\term{f}_3 : \typeformer{F}_3)} \cdots \sum_{(\term{f}_{n-1} : \typeformer{F}_{n-1})}\typeformer{F}_n\right)}{\lambda\,(\type{f}_1,\unitelem,\term{f}_3,\dots), \type{f}_1 \times (\type{f_1} \rightarrow \type{A})}
\end{equation}
where \(\typeformer{F}_2\) has been moved to the last part of the container, and the "\(\rightarrow \typeformer{M}\)" has been removed. We can even leave out the "\(\unit \times\)" and \(\unitelem\) case from the container, since it does not add any information. Another case is that the type of a field is dependent \(\typeformer{F_2} = (\term{x} : \type{f}_1) \rightarrow \typeformer{B}\,\term{x} \rightarrow \typeformer{M}\), but again by currying we get \(\typeformer{F_2} : \sum_{(\term{x} : \type{f}_1)} \typeformer{B}\,\term{x} \rightarrow \mathtt{M}\) which can be defined by the container
\begin{equation}
...
\end{defn}
\noindent We can now construct some more interesting data types by set quotienting \texttt{M}-types, which we call Q\texttt{M}-types. Some examples follow in the next section.

\section{Q\texttt{M}-types and Quotient inductive-inductive types (QIITs)}
In this section we will show some examples of quotiented \texttt{M}-type defined using set truncated quotients. We will also construct the types as a quotient inductive-inductive types instead, and show these two construction give equal types, assuming the axiom of countable choice. A quotient inductive-inductive type (QIIT) \cite{DBLP:QIIT} is a type that is defined at the same time as a relation over that type and then set truncated. We believe, but do not show, that this equality between the two ways of constructing Q\texttt{M}-types is general.
...
\begin{defn}
  ...
  \begin{equation}
    \begin{aligned}
      ...
      &\quad\function{isProp{\rightarrow}isDepProp}~\function{P_{prop}}~(\function{elimProp_{MS}}~(\constructor{node}~\function{f}))~(\function{elimProp_{MS}}~(\constructor{node}~(\function{f} \circ \function{g})))\\[-1mm]
      ...
      &\quad\function{isSet{\rightarrow}isDepSet}~(\function{isProp{\rightarrow}isSet} \circ \function{P_{prop}})~(\function{elimProp_{MS}}~\term{a})~(\function{elimProp_{MS}}~\term{b})\\[-1mm]
      &\quad\qquad(\mathtt{ap}~\function{elimProp_{MS}}~\pathterm{p})~(\mathtt{ap}~\function{elimProp_{MS}}~\pathterm{q})~(\constructor{MS\text{-}isSet}~\term{a}~\term{b}~\pathterm{p}~\pathterm{q})~i~j \\
    \end{aligned}
  \end{equation}
\end{defn}
\begin{lem}
...
\begin{thm}
  ...
  \begin{proof}
    ...
    \begin{equation}
      ...
      \end{aligned}
    \end{equation}
    Now for the other direction from \((\function{s},\function{q})~\relation{\sim_{Seq}}~(\function{t},\function{p})\) to \(\function{Seq{\rightarrow}Delay}~(\function{s},\function{q})~\relation{\sim_{Delay}}~\function{Seq{\rightarrow}Delay}~(\function{t},\function{p})\). We start with some helpful properties. We know that if \(\function{s}_0 = \mathtt{inl}~\term{r}\), then \((\function{s},\function{q}) = \function{now}_{\mathtt{Seq}}~\term{r}\) and \((\function{s},\function{q})~\relation{\downarrow_{Seq}}~\term{r}\). If two sequences are weakly bisimilar and they terminate to different values, then the values are equal, meaning
    \begin{equation}
    ...
    \end{equation}
    We do the proof by case on \(\function{s}_0\) and \(\function{t}_0\). If both sequences are terminated, meaning \((\function{s},\function{q}) = \function{now}_{\typeformer{Seq}}~\term{a}\) and \((\function{t},\function{p}) = \function{now}_{\typeformer{Seq}}~\term{b}\), then we can make a term \(\pathterm{p} : \term{a} \equiv \term{b}\) using \(\function{\sim_{seq\text{-}val}}\). The base case is then done by \(\constructor{\sim_{now}}~\pathterm{p}\). If two sequences are weakly bisimilar, then shifting or unshifting either of them will not break the bisimilarity. We can therefore do the rest of the cases by induction.
    \begin{equation}
      \begin{aligned}
...
\begin{exmp}
  ...
  \end{equation}
  This relation fulfills the \(\function{\sim_{ap}}\) property. We can see this by case. If \(\term{a} = \mathtt{inl}~\term{r}\), then it holds trivially. If \(\term{a} = \mathtt{inr}~\unitelem\), then we have \(\function{f}~\relation{\sim_{PT}}~\function{h}\) and want to show \(\function{k} \circ \function{f}~\relation{\sim_{PT}}~\function{k} \circ \function{h}\) for any function \(\function{k}\), which just boils down to showing \(\function{k} \circ \function{f} \circ \function{g} \equiv \function{k} \circ \function{h}\) given the path \(\pathterm{p} : \function{f} \circ \function{g} \equiv \function{h}\), which can be done by \(\mathtt{ap}~\function{k}~\pathterm{p}\). We therefore have a quotient polynomial functor \(\functor{F}\) for multisets with the \(\function{\sim_{ap}}\) property. We then define \(\function{abs}\) as
  \begin{equation}
    \begin{aligned}

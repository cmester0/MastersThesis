...
\label{ch:intro}
when \todo{...}... a constructor for a type \(\type{A}\) is a function, that takes some arguments, and returns an element \(a : \type{A}\), dually a destructor of \(\type{A}\) will return something given an element \(a : \type{A}\). \todo{...}... from a set of constructors, for example the natural numbers \(\mathbb{N}\) which are defined by the two constructors
\begin{center}
  \strut
  \hfill
...
\\ \\
\todo[inline]{W-types, Induction , M-types, Coinduction..., see introduction?}
...
\label{ch:m-types}
In this chapter we will introduce containers (aka. signatures), and use them to construct \texttt{M}-types and operations \(\function{in}\) and \(\function{out}\) on the \texttt{M}-types (\thmref{M-shift}). We conclude the chapter by defining a coinduction principle for \texttt{M}-types. \todo{Some formalization of coalgebras is missing?}
...
\begin{defn}[Coinduction principle]
  ...
  \end{figure}
  \noindent where \(\pi_1^{\type{\overline{\mathcal{R}}}} = ~! = \pi_2^{\type{\overline{\mathcal{R}}}}\) \todo{what is \(!\)} where \(!\) is the unique mapping property (UMP) out of the final coalgebra. Given \(\term{r} : \term{m}~\relation{\mathcal{R}}~\term{m'}\) we get the equation
  \begin{equation}
...
\end{equation}
the container is \(\containerpairsimple{\unit}{\unit}\) \todo{why gives some arguements or illustations showing these reasonings}. If we want a type with both these rules, then we just take the disjoint union of the two containers
\begin{equation}
...
\todo{define a wacky M-type showing some of the use of M-types / complexity}


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% 

\chapter{QM-types}
In this chapter we will introduce quotients, and show how quotients of \texttt{M}-types, are equal to QIIT. \todo{better introduction to chapter, and reference to main points}
...
Since we know that \texttt{M}-types preserves the H-level, we can use set-truncated quotients, to define quotient \texttt{M}-types, though we run into the problem of \todo{problem of direct quotients}, as a solution we use QIIT to define the relation and type at the same type.
...
\begin{lem}[\(\constant{inl{\not\equiv}inr}\)]
  ...
  \begin{proof}
    The constructors are disjoint, so there is not a path between them \todo{better formulated proof}.
  \end{proof}
f\end{lem}
\begin{defn}
...
\end{equation}
This is enough to define, what we call, silhouette trees, which are trees quotiented by this notion of weak bisimilarity, namely \(\typeformer{tree} / \relation{\sim_{tree}}\). We can also construct this type directly as a QIIT, with type constructors \todo{add all needed constructors}
\begin{equation}
...
\begin{defn}[Sequence Termination]
  ...
  \end{equation}
\end{defn}
We define weak bisimilarity relation for sequences \todo{Is this defined for partiality monad?}
\begin{defn}
  \begin{equation}
...
\end{defn}
\begin{lem}
  \(\function{Seq/{\sim}{\rightarrow}sTree}\) is injective
  \todo{TODO}
\end{lem}
\begin{lem}
  \(\function{Seq/{\sim}{\rightarrow}sTree}\) is surjective
  \todo{TODO}
\end{lem}
\begin{thm}
  \(\function{Seq/{\sim}{\rightarrow}sTree}\) is an equivalence.
  \todo{TODO}
\end{thm}

\subsection{QM-types}
We want to define sequences based on \texttt{M}-types, here are some examples
\begin{align}
  \typeformer{Seq}_{\typeformer{M}_{\containerpair{\type{A}}{\empt}}} &= \type{A} \\
  \typeformer{Seq}_{\typeformer{M}_{\containerpair{\type{A} + \unit}{[\empt,\unit]}}} &= \sum_{(\function{s} : \mathbb{N} \rightarrow \type{A} + \unit)} \prod_{(n : \mathbb{N})} \function{s}_n \sqsubseteq \function{s}_{n+1} \\
  \typeformer{Seq}_{\typeformer{M}_{\containerpair{\unit + \unit}{[\empt,\type{E}]}}} &= \prod_{(\function{e} : \mathbb{N} \rightarrow \type{E})}\sum_{(\function{s} : \mathbb{N} \rightarrow \unit + \unit)} \prod_{(n : \mathbb{N})} \function{s}_n \sqsubseteq \function{s}_{n+1} \\
  \typeformer{Seq}_{\typeformer{M}_{\containerpair{\type{A} + \unit}{[\empt,\type{E}]}}} &= \prod_{(\function{e} : \mathbb{N} \rightarrow \type{E})}\sum_{(\function{s} : \mathbb{N} \rightarrow \type{A} + \unit)} \prod_{(n : \mathbb{N})} \function{s}_n \sqsubseteq \function{s}_{n+1} \\
  \typeformer{Seq}_{\typeformer{M}_{\containerpair{\type{A} + \type{B}}{[\empt,\type{E}]}}} &= \prod_{(\function{e} : \mathbb{N} \rightarrow \type{E})}\sum_{(\function{s} : \mathbb{N} \rightarrow \type{A} + \type{B})} \prod_{(n : \mathbb{N})} \function{s}_n \sqsubseteq \function{s}_{n+1} \\
  \typeformer{Seq}_{\typeformer{M}_{\containerpair{\unit}{\unit + \unit}}} &=~\prod_{(\function{e} : \mathbb{N} \rightarrow \unit + \unit)}\sum_{(\function{s} : \mathbb{N} \rightarrow \type{1})}\prod_{(n : \mathbb{N})} \function{s}_n \sqsubseteq \function{s}_{n+1} \\
  \typeformer{Seq}_{\typeformer{M}_{\containerpair{\type{A}}{\unit + \unit}}} &=~\prod_{(\function{e} : \mathbb{N} \rightarrow \unit + \unit)}\sum_{(\function{s} : \mathbb{N} \rightarrow \type{A})}\prod_{(n : \mathbb{N})} \function{s}_n \sqsubseteq \function{s}_{n+1} \\

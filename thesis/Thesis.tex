...
\begin{lem}
  ...
  \begin{proof}
    ...
    \begin{equation}
      ...
      \end{aligned}
    \end{equation}
    to show the injectivity, meaning for all \(\term{x}~\term{y} : \typeformer{Seq}_{\type{R}} / \relation{\sim_{seq}}\) we get \(\function{Seq/{\sim}{\rightarrow}\bot}~\term{x} \equiv \function{Seq/{\sim}{\rightarrow}\bot}~\term{y} \rightarrow \term{x} \equiv \term{y}\). We start by eliminating \(\term{x}\), followed by elimination of \(y\), this gives us the proof term \todo{Convert to text, instead of a proof term!?}
    \begin{equation}
      \begin{aligned}
        &\function{elimProp} \\

...
\begin{align}
  U &\equiv \sum_{f : C \rightarrow \mathcal{L}} \mathtt{out} \circ f \equiv \mathtt{step}~f \\
    ...
    &\equiv \unit
\end{align}
%     \sum (\sum ((n : ℕ) \rightarrow C \rightarrow X (sequence S) n) (\lambda u \rightarrow u \equiv ϕ₀ {C,γ = C,γ} u))
%       (\lambda { (u , p) \rightarrow \sum ((n : ℕ) \rightarrow π (sequence S) \circ u (suc n) \equiv u n) \lambda q \rightarrow (transport (\lambda j -> p j ? ?) q \equiv ϕ₁ u q)})
%   \equiv⟨ {!!} ⟩
%     {!!}
%   \equiv⟨ {!!} ⟩
%     \sum ((n : ℕ) \rightarrow C \rightarrow X (sequence S) n) (\lambda u \rightarrow \sum (u \equiv ϕ₀ {C,γ = C,γ} u) \lambda p \rightarrow \sum ((n : ℕ) \rightarrow π (sequence S) \circ u (suc n) \equiv u n) \lambda q \rightarrow (\lambda n \rightarrow transport (\lambda i \rightarrow (\lambda x \rightarrow π (sequence S) (p i (suc n) x)) \equiv p i n) (q n)) \equiv ϕ₁ u q)
%   \equiv⟨ {!!} ⟩
%     (\sum (Lift {ℓ-zero} {ℓ} Unit) (\lambda { (lift tt) \rightarrow Lift {ℓ-zero} {ℓ} Unit }))
%   \equiv⟨ isoToPath (iso (\lambda x \rightarrow lift tt) (\lambda _ \rightarrow lift tt , lift tt) (\lambda b i \rightarrow lift tt) (\lambda a i \rightarrow lift tt , lift tt)) ⟩
%     Lift Unit ∎

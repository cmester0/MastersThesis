...
\end{equation}
One way to define this type is as the QM-type given by quotienting \(\typeformer{T}\,\type{A}\) by the relation \(\relation{\sim_T}\) defined by the constructors
\begin{center}
  \strut
  \hfill
  \begin{minipage}{0.35\linewidth}
...
\todo{...} again we run into problems with the axiom of choice. The construction of QW types got around this problem, by doing the construction as a QIIT, we will try and follow the same ideas and show what that would look like for \texttt{M}-types. \todo{Complete construction of QW inspired QM-types}

...
In this section we show some examples of coinductive types that can be constructed as \mathtt{M}-types, and show how to define the constructors/destructors of these types. We conclude this chapter with some general observation, and define some rules for how to construct \texttt{M}-types. \todo{Is there anything else that is show for each M-type?}
...
\todo[inline]{define a wacky \texttt{M}-type showing some of the use of \texttt{M}-types / complexity}
...
\begin{defn}[Eliminator for quotients]
  ...
  Given a statement \(\typeformer{P} : \type{A} / \relation{R} \rightarrow \universe{\mathcal{U}}\) which is a set \(\function{P_{set}} : \prod_{(\term{x} : \type{A}/\relation{R})} \function{isSet}~(\typeformer{P}\,\term{x})\), a proof for the base case \(\function{f} : \prod_{(\term{a} : \type{A})} \typeformer{P}\,\quotientconstructor{\term{a}}\) and a proof for equality \(\function{f_{eq}} : \prod_{(\term{a},\term{b} : A)} \prod_{(\term{r} : \term{a}~\relation{R}~\term{b})} \function{f}~\term{a} \equiv_* \function{f}~\term{b}\) over \(\constructor{eq/}~\term{a}~\term{b}~\term{r}\), we get a function \(\function{elim} : (\term{x} : \type{A} / \relation{R}) \rightarrow \typeformer{P}\,\term{x}\) as follows
  \begin{equation}
    \begin{aligned}
...
In this section we will show some examples of quotiented \texttt{M}-types, and alternative ways of defining an equal type, but using quotient inductive-inductive types instead. A quotient inductive-inductive type (QIIT) is a type defined at the same time as a relation over that type. Furthermore QIITs are set truncated. We belive that every Q\texttt{M}-types is equal (under the axiom of choice) to a QITT, however QIITs are more general than our Q\texttt{M}-types, so we cannot expect every QIIT to have a corresponding Q\texttt{M}-type. \todo{Go into more details!} % QIITs are HIITs that are set truncated. 
...
\todo[inline]{Building the weak bisimulation on the \texttt{M}-type as a \texttt{M}-type - Is this possible? Yes! Should it be included?}

...
\vspace*{\fill}\noindent{\rule{\linewidth}{1mm}\\[4ex]
  ...
  {\huge\sf Lasse Letager Hansen, 201508114}\\[2ex]
  \noindent\rule{\linewidth}{1mm}\\[4ex]
...
\section{Notation}
The notation and fonts used in the rest of the report are described in \tableref{notation}.
\begin{table}[h]
  % \centering
  \begin{tabular}[h]{ll}
...
\begin{thm}
  ...
  \begin{proof}
    By unfolding the definition, we need to show \(\prod_{(\coalg{\type{C}}{\function{\gamma}} : \mathtt{Coalg}_{\container{S}})} \mathtt{isContr}~(\coalg{\type{C}}{\function{\gamma}} \Rightarrow \coalg{\type{\mathcal{L}}}{\function{out}})\). We therefore assume, we are given some coalgebra  \(\coalg{\type{C}}{\function{\gamma}} : \mathtt{Coalg}_{\container{S}}\) and want to show that morphisms into \(\coalg{\type{\mathcal{L}}}{\function{out}}\) are contractible. We do this by showing \((\coalg{\type{C}}{\function{\gamma}} \Rightarrow \coalg{\type{\mathcal{L}}}{\function{out}}) \equiv \unit\). We get
    \begin{align}
...
\end{proof}
\end{thm}
\noindent We can see that the \constructor{now} and \constructor{leaf} constructors have the same definitional structure in the container and diagrammatic structure.
...
\begin{defn}[Sequence Termination]
  The following relation says that a sequence \((\function{s},\function{q}) : \typeformer{Seq}_{\type{R}}\) terminates with a given value \(\term{r} : \type{R}\),
  \begin{equation}
...
\end{exmp}
\noindent We can see that doing set truncated quotients probably removes more information than we would want. We would like to quantify this loss of information, however this is left at future research.

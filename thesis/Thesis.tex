...
\end{exmp}
\begin{defn}
  We define a chain as a familily of cones \(\function{\pi}_{(n)} : \type{X}_{n+1} \rightarrow \type{X}_{n}\), over a family of types \(\type{X}_{n}\). \todo{cocones?}.
  \begin{figure}
    \centering
    \begin{tikzcd}[row sep=large,column sep=large]
    ...
    \end{tikzcd}
  \end{figure}  
  \todo[inline]{define chains, \(\function{\pi}_{(n)}\) and \(\type{X}_n\)}
\end{defn}
...
\begin{lem}
  ...
  \begin{proof}
    We construct an isomorphism by the functions \(\function{tree{\rightarrow}Seq}\) and \(\function{Seq{\rightarrow}tree}\), with right inverse given by two cases, one where the first element in the sequence is \(\mathtt{inl}~\term{r}\), meaning representing a leaf with value \(r\), then we need to show that \(\function{s} \equiv \lambda\,\_,(\lambda\,\_,\mathtt{inl}~\term{r}),(\lambda\,\_,\mathtt{inl}~\mathtt{refl})\) which follows from \eqref{eq:seq-first-is-leaf}. Otherwise we need to show that
    \begin{equation}
    ...
    \begin{equation}
      \begin{aligned}
        &\function{tree\text{-}Seq}~\function{s} = \begin{cases} \lambda\,\_,(\lambda\,\_,\mathtt{inl}~\term{r}),(\lambda\,\_,\mathtt{inl}~\mathtt{refl}) & \prod_{e : \mathbb{N} \rightarrow \type{E}} \pi_1~(\function{s}~\function{e})~0 = \mathtt{inl}~\term{r} \\ \function{shift}~(\function{tree{\rightarrow}Seq} \circ \function{Seq{\rightarrow}tree} \circ \function{unshift}~\function{s}) & o.w. \end{cases} \\
        &\function{tree\text{-}Seq}~\function{s} = \mathtt{refl} \\
        &\function{tree\text{-}Seq}~\function{s} = \mathtt{ap}~\function{shift}~(\function{tree\text{-}Seq}~(\function{unshift}~\function{s})) \ct \function{shift\text{-}unshift}
      \end{aligned}
    \end{equation}
    and left inverse
    \begin{equation}

...
\begin{lem}
  ...
  \begin{proof}
    ...
    \end{equation}
    which is a proposition for all \(\term{x}\) by \(\function{P_{prop}} = \lambda\,\term{x},~\function{isProp\Pi}~(\lambda\,\_, \constructor{squash/}~\term{x}~\term{y})\). We do the propositional elimination twice, first for \(\term{x}\) and then for \(\term{y}\), where we define \(\typeformer{P}\) and \(\function{P_{prop}}\) similarly, but with \(\term{x} = \quotientconstructor{\term{a}}\) since it has already been propositionally eliminated. 
    \begin{equation}
      \begin{aligned}
...
\end{defn}
\begin{lem}
  The function \(\function{T/{\sim}{\rightarrow}MS}\) is surjective \((\prod_{(\term{b} : \typeformer{MS}\,\type{X})}, \| \Sigma_{(\term{x} : \typeformer{T}\,\type{X} / \relation{\sim_{T}})}~\function{T/{\sim}{\rightarrow}MS}~\term{x} \equiv \term{b} \|)\), assuming the axiom of countable choice.
  \begin{proof}
    We do propositional elimination of \(\typeformer{MS}\), with \(\typeformer{P}~\term{y} = \| \Sigma_{(\term{x} : \typeformer{T}\,\type{X} / \relation{\sim_{T}})}~\function{T/{\sim}{\rightarrow}MS}~\term{x} \equiv \term{y} \| \), which is a proposition by truncation. For the leaf case, we have the simple equality \(\function{T/{\sim}{\rightarrow}MS}~\quotientconstructor{\constructor{leaf}_{\typeformer{T}}~\term{x} / \relation{\sim_{T}}} \equiv \constructor{leaf}_{\typeformer{MS}}~\term{x}\). For the node case with \(\constructor{node}~\function{f}\), where we get the following function by induction
    \begin{equation}
    ...
    \begin{align}
      &\quad~ \function{T/{\sim}{\rightarrow}MS}~\quotientconstructor{\constructor{node}~\function{g}} \\
      &\equiv \function{T{\rightarrow}MS}~(\constructor{node}~\function{g}) \\
      &\equiv \constructor{node}~(\function{T{\rightarrow}MS} \circ \function{g}) \\
      &\equiv \constructor{node}~\function{f}
    \end{align}
    completing the case for node and the proof.
  \end{proof}
\end{lem}
\begin{thm}
...
\begin{thm}
  ...
  \begin{proof}
    ...
    \begin{equation}
      ...
      \end{aligned}
    \end{equation}
    We show if \((\function{s},\function{q})~\relation{\sim_{Seq}}~(\function{t},\function{p})\) then \(\function{Seq{\rightarrow}Delay}~(\function{s},\function{q})~\relation{\sim_{Delay}}~\function{Seq{\rightarrow}Delay}~(\function{t},\function{p})\) by case on \(\function{s}~0\) and \(\function{t}~0\). We know that if \(\function{s}~0 = \mathtt{inl}~\term{r}\), then \((\function{s},\function{q}) = \function{now}_{\mathtt{Seq}}~r\) and \((\function{s},\function{q})~\relation{\downarrow_{Seq}}~\term{r}\). If two sequences are weakly bisimilar and they terminate to different values, then the values are equal, we therefore get
    \begin{equation}
    ...
    \end{equation}
    If both sequences are terminated meaning \((\function{s},\function{q}) = \function{now}_{\typeformer{Seq}}~\term{a}\) and \((\function{t},\function{p}) = \function{now}_{Seq}~\term{b}\), then \(\pathterm{p} : \term{a} \equiv \term{b}\) by \(\function{\sim_{seq\text{-}val}}\), we can therefore construct \(\constructor{\sim_{now}}~\pathterm{p}\) completing the base case. If two sequences are weakly bisimilar, then shifting or unshifting either of them will not break the bisimilarity, which makes the rest of the cases follow by induction.
    \begin{equation}
      \begin{aligned}
    ...
    \end{aligned}
  \end{equation}
  The right and left identity for \(\function{{\sim}_{Delay}{\rightarrow}{\sim}_{Seq}}\) and \(\function{{\sim}_{Seq}{\rightarrow}{\sim}_{Delay}}\) follow from similar arguments. We thereby get that weakly bisimilar relations for the delay monad and sequences respects the equality given in \thmref{seq-eq-delay}, meaning that the quotiented types are also equal.
  \end{proof}
\end{thm}
...
\end{defn}
\begin{lem}
  The function \(\function{Seq{\rightarrow}(-)_{\bot}}\) is surjective, assuming the axiom of countable choice.
  \begin{proof}
    We do the proof by using the propositional eliminator for the partiality monad, with
    \begin{equation}
      \typeformer{P} = \lambda \term{y}, \bigg\| \sum_{(\term{x} : \typeformer{Seq}_{\type{R}})} \function{Seq{\rightarrow}(-)_{\bot}}~\term{x} \equiv \term{y} \bigg\|
    \end{equation}
  ...
  \end{equation}
  which is a pointwise equivalence, which gives us an equivalence for the upper bound
  \begin{align} 
    &\quad~ {\textstyle{\bigsqcup}}(\function{s},\function{q}) \\
    &\equiv \function{Maybe{\rightarrow}(-)_{\bot}} \circ \pi_1 \circ \function{f} , \function{Maybe{\rightarrow}(-)_{\bot}\text{-}mono} \circ \pi_2 \circ \function{f} \\
    &\equiv \function{Maybe{\rightarrow}(-)_{\bot}} \circ \pi_1 \circ \function{f} , \function{Maybe{\rightarrow}(-)_{\bot}\text{-}mono} \circ \pi_2 \circ \function{f} \\
    &\equiv \function{Seq{\rightarrow}(-)_{\bot}} \circ \function{f}
  \end{align}
  \begin{equation}
    \bigg\| \sum_{(\function{x} : \typeformer{Seq}_{\type{R}})} \function{Seq{\rightarrow}(-)_{\bot}}~\term{x} \equiv {\textstyle{\bigsqcup}}(\function{s},\function{q})) \bigg\|
  \end{equation}
...
\begin{thm}
  ...
  Sequences quotiented by weak bisimilarity is equivalent to the partiality monad \(Seq_{\type{R}} / \relation{\sim_{seq}} \equiv \type{R}_{\bot}\), assuming the axiom of countable choice.
  \begin{proof}

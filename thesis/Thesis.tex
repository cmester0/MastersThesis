...
We define
\begin{equation}
  \mathtt{Seq}_A = \sum_{(\function{g} : \type{\mathbb{N}} \rightarrow \type{A} + \unit)} \prod_{(n : \type{\mathbb{N}})} (\function{g}_n \equiv \function{g}_{n+1})  + ((\function{g}_n \equiv \mathtt{inr}~\constant{tt}) \times (\function{g}_{n+1} \not\equiv \mathtt{inr}~\constant{tt}))
\end{equation}
...
\end{equation}
is the identity function. Similarly if we shift followed by an unshift, we just introduce a value to instantly remove it, meaning the value does not matter, again the function
\begin{equation}
  \lambda\,(\function{g},\function{a}),~\function{unshift}~(\function{shift}~(\function{g},\function{a})~\_)
\end{equation}
is the identity function. We can now define an equivalence between \(\typeformer{delay}~\type{R}\) and \(\typeformer{Seq}_{\type{R}}\), where later are equivalent to shifts, and \(\function{now}~\term{r}\) is equivalent to the infinite sequence of \(\mathtt{inl}~\term{r}\). We define
\begin{equation}
  \begin{aligned}
    \function{j} &: \typeformer{Delay}~R \rightarrow \typeformer{Seq}_A \\
    \function{j}&~(\function{now}~\term{a}) = (\lambda\,\_,\mathtt{inl}~\term{a}) , (\lambda\,\_,\mathtt{inl}~\mathtt{refl}) \\
    \function{j}&~(\function{later}~\term{x}) = \function{shift}~(\function{j}~\term{x})~\_
  \end{aligned}
\end{equation}
...
\begin{equation}
  \begin{aligned}
    ...
    \function{h}&~(\function{g} , \function{q}) = \begin{cases} \function{now}~\term{r} & \function{g}_0 = \mathtt{inl}~\term{r} \\ \function{later}~(\function{h}~(\function{unshift}~\term{g})) & \function{g}_0 = \mathtt{inr}~\constant{tt} \end{cases}
  \end{aligned}
\end{equation}
with the right and left identities
\begin{equation}
  \begin{aligned}
    \function{j\text{-}h} &: \forall\,(\function{g},\function{q}) \rightarrow \function{j}~(\function{h}~(\function{g},\function{q})) \equiv (\function{g},\function{q}) \\
    \function{j\text{-}h}&~(\function{g},\function{q}) = \begin{cases} \mathtt{refl}  & \function{g}_0 = \mathtt{inl}~\term{r} \\ \mathtt{cong}~\function{shift}~(\function{j\text{-}h}~(\function{unshift}~(\function{g} , \function{q}))) \ct \function{shift\text{-}unshift}~(\function{g} , \function{q}) & \function{g}_0 = \mathtt{inr}~\constant{tt} \end{cases}
  \end{aligned}
\end{equation}
\begin{equation}
  \begin{aligned}
...
\todo[inline]{Quotient the relation and Seq by weak bisimilarity, to get a partial order with correct equality in \(\alpha\)!}

...
Final {S = S} = Σ[ X,ρ ∈ Coalg₀ {S = S} ] ∀ (C,γ : Coalg₀ {S = S}) → isContr ((C,γ) ⇒ (X,ρ))

--------------------------------------------------------
-- Properties of Bisimulations and (Final) Coalgebras --
--------------------------------------------------------

U : ∀ {ℓ} {S : Container ℓ} (C,γ : Coalg₀ {S = S}) → Type ℓ
U {S = S} (C , γ) =
...
U-is-Unit-Iso {ℓ = ℓ} {S = S} C,γ@(C , γ) =
  ...
  (Σ[ (u , q) ∈ Cone C,γ ]
  (Σ[ p ∈ u ≡ ϕ₀ u ] (PathP (λ i → Cone₁ {C,γ = C,γ} (p i)) q (ϕ₁ u q))))
    Iso⟨ (iso (λ {((u , p) , q , r) → (u , q) , p , r}) (λ {((u , q) , p , r) → (u , p) , (q , r)}) (λ _ → refl) λ _ → refl) ⟩
  (Σ[ (u , p) ∈ (Σ[ u ∈ Cone₀ {C,γ = C,γ} ] (u ≡ ϕ₀ u)) ]
  (Σ[ q ∈ Cone₁ {C,γ = C,γ} u ] (PathP (λ i → Cone₁ {C,γ = C,γ} (p i)) q (ϕ₁ u q))))
  ...
  where
    ...
    missing-2-Iso :
      Iso
        (Lift {ℓ-zero} {ℓ} Unit)
        (Σ[ q ∈ (Cone₁ {C,γ = C,γ} (mi .fst)) ]
          (PathP (λ i → Cone₁ {C,γ = C,γ} (mi .snd i))
            q
            (ϕ₁ (mi .fst) q)))
    missing-2-Iso =
      Lift Unit
      ...
      (Σ[ q ∈ (Cone₁ {C,γ = C,γ} (mi .fst)) ] ((n : ℕ) → q (suc n) ≡ ϕ₁ (mi .fst) q (suc n)))
        Iso⟨ Σ-ap-iso₂ (λ x → idIso) ⟩
      (Σ[ q ∈ (Cone₁ {C,γ = C,γ} (mi .fst)) ] ((n : ℕ) →
      ...
      (Σ[ q ∈ (Cone₁ {C,γ = C,γ} (mi .fst)) ] (
        ...
        Iso⟨ Σ-ap-iso₂ (λ x →
             iso (λ {a i 0 → refl ; a i (suc n) → a i n})
                 ...
                 λ _ → refl) ⟩
      (Σ[ q ∈ (Cone₁ {C,γ = C,γ} (mi .fst)) ] (PathP (λ i → Cone₁ {C,γ = C,γ} (mi .snd i))
                                             q
                                             (ϕ₁ (mi .fst) q))) ∎Iso

U-contr : ∀ {ℓ} {S : Container ℓ} (C,γ : Coalg₀ {S = S}) → isContr (U C,γ)
...
coalg-unfold : ∀ {ℓ} {S : Container ℓ} → (C,γ : Coalg₀ {S = S}) → (_⇒_ {S = S} (C,γ) (M-coalg {S = S}))
coalg-unfold C,γ = lim-terminal {C,γ = C,γ} .fst

-- unique function into final coalg
coalg-unfold-universal : ∀ {ℓ} {S : Container ℓ} → (C,γ : Coalg₀ {S = S}) → (y : C,γ ⇒ M-coalg) → fst lim-terminal ≡ y
coalg-unfold-universal C,γ = lim-terminal {C,γ = C,γ} .snd

-- unique function into final coalg
coalg-unfold-function : ∀ {ℓ} {S : Container ℓ} → (C,γ : Coalg₀ {S = S}) → (C,γ .fst) → (M-coalg .fst)

...
stream-S : ∀ (A : Type₀) -> Container ℓ-zero
stream-S A = (A , (λ _ → Unit))

stream : ∀ (A : Type₀) -> Type₀
stream A = M (stream-S A)

cons : ∀ {A : Type₀} -> A -> stream A -> stream A
cons x xs = in-fun (x , λ { tt -> xs } )

hd : ∀ {A : Type₀} -> stream A -> A
hd {A} S = out-fun S .fst

tl : ∀ {A : Type₀} -> stream A -> stream A
...
-- stream-expand-2 : ∀ {A : Type₀} {s t : stream A} → (s ≡ t) ≡ (cons (hd s) (tl s) ≡ cons (hd t) (tl t))
-- stream-expand-2 = sym in-out-id

-- stream-expand-1 : ∀ {A : Type₀} {a c : A} {b d : stream A} → (cons a b ≡ cons c d) ≡ (a ≡ c) × (b ≡ d)
...
-- stream-expand : ∀ (A : Type₀) (s t : stream A) → (s ≡ t) ≡ (hd s ≡ hd t) × (tl s ≡ tl t)
-- stream-expand A s t = stream-expand-2 ∙ stream-expand-1

-- hd-is-hd : ∀ (A : Type₀) (x : A) (xs : stream A) → hd (cons x xs) ≡ x
-- hd-is-hd A x xs =
--   hd (cons x xs)
--     ≡⟨ cong (λ a → a .fst) (funExt⁻ (out-inverse-in {S = stream-S A}) (x , λ { tt → xs })) ⟩
--   x ∎

-- tl-is-tl : ∀ (A : Type₀) (x : A) (xs : stream A) → tl (cons x xs) ≡ xs
-- tl-is-tl A x xs =
--   tl (cons x xs)
...
stream-pair-M : ∀ (A B : Type₀) → stream A × stream B ≡ M (Container-product (stream-S A) (stream-S B))
stream-pair-M A B = M-product-equality (stream-S A) (stream-S B)

Container-product-streams : ∀ (A B : Type₀) → Container-product (stream-S A) (stream-S B) ≡ stream-S (A × B)
...
stream-pair : ∀ (A B : Type₀) → stream A × stream B ≡ stream (A × B)
stream-pair A B = stream-pair-M A B ∙ λ i → M (Container-product-streams A B i)

zip : ∀ {A B : Type₀} → stream A × stream B → stream (A × B)
...
Stream-to-stream-func-x : ∀ {A : Type₀} (n : ℕ) -> Stream A → X (sequence (stream-S A)) n
Stream-to-stream-func-x 0 x = lift tt
Stream-to-stream-func-x (suc n) x = head x , λ _ → Stream-to-stream-func-x n (tail x)

Stream-to-stream-func-π : ∀ {A : Type₀} (n : ℕ) (a : Stream A) → π (sequence (stream-S A)) (Stream-to-stream-func-x (suc n) a) ≡ Stream-to-stream-func-x n a
...
-- tl-to-tail : ∀ {A : Type₀} (b : Stream A) → tl (Stream-to-stream b) ≡ Stream-to-stream (tail b)
-- tl-to-tail {A = A} b =
...
postulate
  tl-to-tail : ∀ {A : Type₀} (b : Stream A) → tl (Stream-to-stream b) ≡ Stream-to-stream (tail b) -- should this be able to compute ??
...
postulate
  stream-equality-iso-2₁ : ∀ {A : Type₀} → (a : stream A) (n : ℕ) → Stream-to-stream (stream-to-Stream a) .fst n ≡ a .fst n
